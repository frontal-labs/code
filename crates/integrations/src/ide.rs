    ...
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Self::ExtensionSourceNotFound(path) => write!(
                f,
                "FrontalCode IDE extension source not found at {}",
                path.display()
            ),
            ...
            Self::InstallFailed { target, details } => {
                write!(f, "failed to install FrontalCode IDE extension into '{target}': {details}")
            }
...
pub fn setup_editor_integration(
    ...
    let payload = serde_json::json!({
        ...
        "cliPath": "frontal-code",
        "notes": "This file is generated by /ide for FrontalCode IDE extension integration."
    });
...
fn editor_config_path(workspace_root: &Path, target: IdeTarget) -> PathBuf {
    match target {
        IdeTarget::Vscode => workspace_root.join(".vscode").join("frontal-code.json"),
        IdeTarget::Cursor => workspace_root.join(".cursor").join("frontal-code.json"),
        IdeTarget::Antigravity => workspace_root
            .join(".antigravity")
            .join("frontal-code.json"),
        IdeTarget::Windsurf => workspace_root.join(".windsurf").join("frontal-code.json"),
    }
}
...
fn render_vsix_manifest(metadata: &ExtensionPackageJson) -> String {
    ...
    let description = metadata
        .description
        .as_deref()
        .unwrap_or("FrontalCode IDE extension");
    format!(
...
mod tests {
    ...
    fn temp_path(label: &str) -> PathBuf {
        ...
        std::env::temp_dir().join(format!(
            "frontal-code-ide-integration-{label}-{nanos}-{pid}-{serial}"
        ))
    }
    ...
    fn status_reads_configured_target() {
        ...
        assert!(status
            ...
            .ends_with(".cursor/frontal-code.json"));
    ...
    fn setup_editor_integration_writes_workspace_config() {
        ...
        assert!(config_path.ends_with(".vscode/frontal-code.json"));
    ...
    fn package_extension_builds_vsix_from_local_extension_source() {
        ...
        let extension_root = root.join("extensions").join("frontal-code-ide");
        fs::create_dir_all(&extension_root).expect("extension root should create");
        fs::write(
            extension_root.join("package.json"),
            r#"{
  "name": "frontal-code-ide",
  "publisher": "frontal-code",
  "version": "0.1.0",
  "displayName": "FrontalCode IDE",
  "description": "FrontalCode integration for VS Code and Cursor"
}"#,
        )
        ...
        fs::write(extension_root.join("README.md"), "# FrontalCode IDE")
            .expect("readme should write");

        let packaged = package_extension(&root).expect("vsix should package");
        assert!(packaged.ends_with(".frontal-code/extensions/frontal-code-ide-0.1.0.vsix"));
