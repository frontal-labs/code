use frontal_code_telemetry::{
    AnthropicRequestProfile, ClientIdentity, DEFAULT_AGENTIC_BETA, DEFAULT_FRONTAL_CODE_VERSION,
...
fn new_creates_profile_with_default_betas() {
    ...
    assert_eq!(profile.anthropic_version, DEFAULT_FRONTAL_CODE_VERSION);
...
fn header_pairs_includes_version_and_user_agent() {
    ...
    assert!(headers.contains(&(
        ...
        DEFAULT_FRONTAL_CODE_VERSION.to_string()
    )));
...
fn header_pairs_empty_betas_omits_beta_header() {
    let profile = AnthropicRequestProfile {
        anthropic_version: DEFAULT_FRONTAL_CODE_VERSION.to_string(),
