# Frontal Code Slack Extension API Reference

## Overview

The Frontal Code Slack extension provides both HTTP endpoints for Slack integration and client APIs for interacting with the Frontal Code server.
...
## Frontal Code API Client

### Task Management

#### createTask

Creates a new hosted task in Frontal Code.
...
```typescript
interface FrontalCodeEventClient {
...
**Example:**
```typescript
const client = new FrontalCodeEventClient(apiClient);
...
### Frontal Code API Authentication

The extension uses API tokens for Frontal Code API authentication:
...
```typescript
class MockFrontalCodeAPIClient implements FrontalCodeAPIClient {
