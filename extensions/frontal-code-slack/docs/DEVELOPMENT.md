# Frontal Code Slack Extension Development Guide

## Overview

This guide covers development setup, coding standards, testing practices, and contribution guidelines for the Frontal Code Slack extension.
...
```
extensions/frontal-code-slack/
src/
  index.ts              # Application entry point
  slack.ts              # Slack client and command handling
  api-client.ts         # Frontal Code API client
  frontal-code-events.ts       # WebSocket event client
  config.ts             # Configuration management
  env.ts                # Environment validation
  log.ts                # Logging configuration
  types.ts              # Type definitions
  generated/
    frontal-code-events.ts      # Generated Frontal Code event types
scripts/
  check-frontal-code-events.mjs # Frontal Code events sync check
tests/
  unit/                 # Unit tests
...
export class SlackClient {
  private app: App;
  private apiClient: FrontalCodeAPIClient;
...
#### Frontal Code API Client (`src/api-client.ts`)

Manages Frontal Code server communication:
...
#### Event Client (`src/frontal-code-events.ts`)
...
```typescript
export class FrontalCodeEventClient {
  ...
  private handleEvent(event: FrontalCodeEvent): Promise<void>;
}
```
...
export class FrontalCodeAPIError extends Error {
  constructor(
    message: string,
    public code: string,
...
// Export classes
export { FrontalCodeAPIClient, SlackClient, FrontalCodeEventClient };
...
describe('FrontalCodeAPIClient', () => {
  let client: FrontalCodeAPIClient;
  let mockFetch: ReturnType<typeof vi.fn>;

  beforeEach(() => {
    mockFetch = vi.fn();
    global.fetch = mockFetch;
    client = new FrontalCodeAPIClient('http://test.com');
  });
...
import { MockFrontalCodeServer } from '../fixtures/mock-frontal-code-server';

describe('Slack Integration', () => {
  let slackClient: SlackClient;
  let mockServer: MockFrontalCodeServer;

  beforeAll(async () => {
    mockServer = new MockFrontalCodeServer();
    ...
    slackClient = new SlackClient({
      ...
      frontal-codeApiUrl: mockServer.url
    });
...
```typescript
// tests/fixtures/mock-frontal-code-server.ts
export class MockFrontalCodeServer {
  private server: any;
  public tasks: Task[] = [];
  public url: string;
...
# Tag and push
docker tag frontal-code-slack:v0.1.0 your-registry/frontal-code-slack:v0.1.0
docker push your-registry/frontal-code-slack:v0.1.0

# Deploy
kubectl set image deployment/frontal-code-slack \
  frontal-code-slack=your-registry/frontal-code-slack:v0.1.0
```

This development guide provides comprehensive guidelines for contributing to the Frontal Code Slack extension.
