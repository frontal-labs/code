# Frontal Code Slack Extension Documentation

## Overview

The Frontal Code Slack extension provides autonomous engineering capabilities directly within Slack, enabling teams to create, manage, and monitor coding tasks through natural language commands.

## Features

- **Task Creation**: Create hosted Frontal Code tasks from Slack messages and slash commands
...
- **Frontal Code API Client** (`src/api-client.ts`): HTTP client for tasks, approvals, and callbacks
- **Event Stream Client** (`src/frontal-code-events.ts`): WebSocket client for real-time task events
- **Configuration** (`src/config.ts`, `src/env.ts`): Environment validation and settings
- **Type Definitions** (`src/types.ts`): Slack and Frontal Code API type definitions

### Data Flow

1. Slack command/message received
2. Task created via Frontal Code API
...
- `/ai <prompt>`: Create a hosted Frontal Code task with the given prompt
...
- Non-bot messages automatically create hosted Frontal Code tasks
...
- Mock Frontal Code server for testing
...
docker build -t frontal-code-slack .

# Run the container
docker run -p 3000:3000 --env-file .env frontal-code-slack
```
...
### Frontal Code API Integration

The extension integrates with several Frontal Code API endpoints:
