# Frontal Code Slack Extension Troubleshooting Guide

## Overview

This guide provides comprehensive troubleshooting steps for common issues with the Frontal Code Slack extension.
...
1. **Monitor Memory Usage**
   ...
   ps aux | grep frontal-code-slack
   
   # Monitor memory over time
   watch -n 5 'ps aux | grep frontal-code-slack'
   ```
...
4. **Check for Leaks**
   ...
   while true; do
     echo "$(date): $(ps -o pid,vsz,rss,comm -p $(pgrep frontal-code-slack))"
     sleep 30
   done
   ```
...
nslookup frontal-code-api.your-domain.com

# Test TCP connectivity
telnet api.slack.com 443
telnet frontal-code-api.your-domain.com 8788
...
- name: frontal-code-slack
  rules:
  - alert: HighErrorRate
    expr: rate(frontal-code_slack_errors_total[5m]) > 0.1
    for: 2m
    labels:
      severity: warning
    annotations:
  ...
  - alert: HighMemoryUsage
    expr: frontal-code_slack_memory_usage_bytes > 500000000
    for: 5m
    labels:
      severity: critical
    annotations:
...
app.get('/health/detailed', async (req, res) => {
  const checks = {
    frontal-code_api: await checkFrontalCodeAPI(),
...
# Test Frontal Code API
curl -f http://localhost:3000/health/frontal-code || exit 1

echo "All health checks passed"
```

This comprehensive troubleshooting guide should help diagnose and resolve most common issues with the Frontal Code Slack extension.
