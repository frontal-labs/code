...
export class SlackInterface {
  ...
  private async handleFrontalCodeTaskEvent(
    ...
    const message = this.formatFrontalCodeEvent(event, nextTask);
    if (!message) {
      return;
    }
    ...
    if (event.event === "approval.resolved") {
      ...
      if (approvalTs) {
        ...
        if (this.isTerminalFrontalCodeEvent(event)) {
  ...
  private buildExternalStatusMessage(
    ...
    const formatted = this.formatFrontalCodeEvent(event, syntheticTask);
  ...
  private parseOrphanPolicyCommand(
    ...
    const query: FrontalCodeOrphanPolicyQuery = {};
  ...
  private async syncTrackedTasksFromFrontalCode(): Promise<void> {
    try {
      ...
      logger.warn("Failed to synchronize Slack tasks from Frontal Code", {
        error: (error as Error).message,
      });
    }
  }

  private toTrackedTask(task: FrontalCodeTask): FrontalCodeTrackedTask | null {
    if (!task.channel_id) {
      return null;
    }
  ...
  private isTerminalFrontalCodeEvent(event: FrontalCodeEventEnvelope): boolean {
    return (
  ...
  private upsertTrackedTask(task: FrontalCodeTrackedTask): void {
  ...
  private syncDerivedApprovalState(task: FrontalCodeTask): void {
