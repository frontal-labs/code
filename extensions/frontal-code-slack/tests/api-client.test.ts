...
import type { FrontalCodeTask, FrontalCodeUpdateTaskContextRequest, SlackBlock, SlackBody } from '../src/types';
...
describe('FrontalCodeApiClient', () => {
  ...
  beforeEach(() => {
    client = new FrontalCodeApiClient();
    mockHttpClient = {
      get: vi.fn(),
      post: vi.fn(),
    };
  ...
  it('upgrades https hosted API URLs to wss for the events stream', () => {
    const secureClient = new FrontalCodeApiClient();
    Object.defineProperty(secureClient, 'baseUrl', {
      value: 'https://frontal-code.example.com/api',
      configurable: true,
    });
  ...
  it('wraps prompt submission failures with an Frontal Code API error', async () => {
  ...
  it('wraps CLI command failures with an Frontal Code API error', async () => {
  ...
  it('wraps version lookup failures with an Frontal Code API error', async () => {
    mockHttpClient.get.mockRejectedValue(new Error('version unavailable'));

    await expect(client.getVersion()).rejects.toThrow('Frontal Code API error: version unavailable');
  });

  it('wraps hosted status, sandbox, and version failures with an Frontal Code API error', async () => {
    mockHttpClient.get
  ...
  it('looks up a single task and the effective orphan policy', async () => {
    ...
    await expect(client.getOrphanPolicy({ repository: 'frontal-code/slack' })).resolves.toEqual(policy);
  ...
  it('posts task context updates to the hosted task context route', async () => {
    const request: FrontalCodeUpdateTaskContextRequest = {
      taskId: 'task-123',
      source: 'slack',
      user_id: 'U123',
      channel_id: 'C123',
  ...
  it('builds a secure hosted events websocket URL from an https base URL', () => {
    const secureClient = new FrontalCodeApiClient();
    Object.defineProperty(secureClient, 'baseUrl', {
      value: 'https://frontal-code.example.com/internal/api/',
      configurable: true,
    });
