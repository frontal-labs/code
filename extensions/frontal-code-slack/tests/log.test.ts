...
describe("log helpers", () => {
  ...
  it("forwards debug/info/warn logs and captures production warnings in Sentry", async () => {
    ...
    expect(innerLogger.debug).toHaveBeenCalledWith("debug message", {
      scope: "debug",
    });
    expect(innerLogger.info).toHaveBeenCalledWith("info message", {
      scope: "info",
    });
  ...
  it("logs health checks to info or error depending on status", async () => {
    ...
    module.logHealthCheck("frontal-code", "unhealthy", { reason: "timeout" });
