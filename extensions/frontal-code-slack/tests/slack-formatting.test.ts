...
type TestableSlackInterface = SlackInterface & {
  ...
  formatLaneBlockedEvent(taskLabel: string, event: FrontalCodeEventEnvelope): {
    text: string;
  };
...
describe('SlackInterface formatting helpers', () => {
  ...
  it('renders orphan policy previews with effective policy and rule details', () => {
    ...
    const response = slack.buildOrphanPolicyCommandResponse({
      ...
      effective_policy: {
        source: 'rule',
        match_repository: 'frontal-code/slack',
        match_source: 'slack',
  ...
  it('formats orphaned approval requests with retry and cancel actions', () => {
    const slack = createSlackInterface();
    const event: FrontalCodeEventEnvelope = {
      event_id: 'evt-123',
      topic: 'approval',
    ...
    const task: FrontalCodeTrackedTask = {
      taskId: 'task-123',
      channelId: 'C123',
  ...
  it('formats non-orphan approval requests as a simple waiting message', () => {
    ...
    expect(message).toEqual({
      text: 'Task task-123 (frontal-code/slack) is waiting for approval.',
    });
  });
  ...
  it('formats blocked lane events with orphan policy detail in the message text', () => {
    ...
    const message = slack.formatFrontalCodeEvent(
      {
        event_id: 'evt-125',
        topic: 'lane',
        event: 'lane.blocked',
  ...
  it('formats cancelled task events with orphan policy detail when present', () => {
    ...
    expect(message).toEqual({
      text: 'Task task-123 (frontal-code/slack) was cancelled. Policy: rule (repo=frontal-code/slack); approval 60s, auto-cancel 300s.',
    });
  });
  ...
  it('formats task-bound github connector events for PR updates, merges, reviews, and comments', () => {
    ...
    const prUpdate = slack.formatFrontalCodeEvent(
      {
        event_id: 'evt-133',
        topic: 'connector',
        event: 'connector.event.received',
    ...
    const merged = slack.formatFrontalCodeEvent(
      {
        event_id: 'evt-133a',
        topic: 'connector',
        event: 'connector.event.received',
    ...
    const review = slack.formatFrontalCodeEvent(
      {
        event_id: 'evt-133b',
        topic: 'connector',
        event: 'connector.event.received',
    ...
    const comment = slack.formatFrontalCodeEvent(
      {
        event_id: 'evt-134',
        topic: 'connector',
        event: 'connector.event.received',
  ...
  it('falls back to a generic update message for unrecognized event types', () => {
    ...
    expect(message).toEqual({
      text: 'Task task-123 (frontal-code/slack) updated: task.reconciled',
    });
  });
  ...
  it('formats lane started events with explicit role and worker details', () => {
    ...
    expect(message).toEqual({
      text: 'Task task-123 (frontal-code/slack) reviewer started with worker ready_for_prompt (worker-123).',
    });
  });
  ...
  it('reads empty task summaries when the event has no payload and ignores invalid task statuses', () => {
    const slack = createSlackInterface();
    const noPayloadMessage = slack.formatFrontalCodeEvent(
      {
        event_id: 'evt-127',
        topic: 'memory',
