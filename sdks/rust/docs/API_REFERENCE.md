...
## Module: `frontal-code_sdk`

### `Frontal Code`

#### `Frontal Code::new(config: FrontalCodeConfig) -> Frontal Code`
...
#### `Frontal Code::start_thread(&self, options: ThreadOptions) -> Result<Thread, FrontalCodeError>`
Start a new conversation thread.

#### `Frontal Code::resume_thread(&self, thread_id: &str, options: ThreadOptions) -> Result<Thread, FrontalCodeError>`
...
#### `Thread::run(&self, input: &ThreadInput, options: &ThreadRunOptions) -> Result<TurnResult, FrontalCodeError>`
Run a turn and buffer all events until completion. Returns final result.

#### `Thread::run_streamed(&self, input: &ThreadInput, options: &ThreadRunOptions) -> Result<StreamedTurn, FrontalCodeError>`
...
pub struct StreamedTurn {
    pub events: Pin<Box<dyn Stream<Item = Result<StreamEvent, FrontalCodeError>> + Send>>,
}
```
...
```rust
pub struct FrontalCodeConfig {
    pub command: Option<String>,           // default: "frontal-code"
