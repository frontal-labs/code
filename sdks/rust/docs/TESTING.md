...
Instead, tests use a mock CLI script (`tests/mock_frontal-code.sh`) that emits
...
```bash
# Run all tests
cargo test --package frontal-code-sdk

# Run with output
cargo test --package frontal-code-sdk -- --nocapture

# Run specific test
cargo test --package frontal-code-sdk test_thread_run -- --nocapture
```
...
```
tests/
├── mock_frontal-code.sh          # Mock CLI - emits JSONL events
...
The mock CLI (`tests/mock_frontal-code.sh`) reads stdin for the request payload
...
### 1. Add test events to `mock_frontal-code.sh`
...
async fn test_my_new_scenario() {
    ...
    let thread = frontal-code.start_thread(ThreadOptions::default()).unwrap();
...
### `TestFrontalCodeBuilder` (in `tests/common.rs`)

```rust
let frontal-code = TestFrontalCodeBuilder::new()
...
```rust
use frontal-code_sdk::test_utils::{assert_event_seq, assert_final_response};
...
- uses: actions-rs/cargo@v1
  with:
    command: test
    args: --package frontal-code-sdk --workspace
```

## Coverage

```bash
cargo llvm-cov --package frontal-code-sdk --html
```