//! Spawning the `frontal-code` CLI and parsing its JSONL event stream.
...
use crate::protocol::{FrontalCodeEvent, ThreadItem, TurnResult, Usage};
use serde_json::Value;

/// Errors surfaced while talking to the `frontal-code` CLI.
#[derive(Debug, thiserror::Error)]
pub enum FrontalCodeError {
  #[error("failed to spawn frontal-code cli: {0}")]
  Spawn(#[from] std::io::Error),
  #[error("frontal-code cli exited with code {code:?}: {stderr}")]
...
fn spawn_and_stream(
  ...
  env: EnvMap,
  cwd: Option<&PathBuf>,
) -> Result<(mpsc::Receiver<FrontalCodeEvent>, Child, Arc<Mutex<String>>), FrontalCodeError> {
...
/// Parse a single JSONL event line from the CLI into an [`FrontalCodeEvent`].
///
...
fn parse_event(line: &str) -> Option<FrontalCodeEvent> {
  let trimmed = line.trim();
  if trimmed.is_empty() {
    return None;
  }
  ...
  match type_str {
    "turn.started" => Some(FrontalCodeEvent::TurnStarted),
    "item.completed" => {
      let item = parse_item(obj.get("item")?)?;
      Some(FrontalCodeEvent::ItemCompleted { item })
    }
    "turn.completed" => {
      ...
      let session_id = obj
        ...
        .map(str::to_string);
      Some(FrontalCodeEvent::TurnCompleted {
        final_response,
        usage,
        session_id,
      })
    ...
    "turn.failed" => {
      ...
      Some(FrontalCodeEvent::TurnFailed { error })
    }
    _ => None,
  }
}
...
pub async fn run_turn_buffered(
  ...
  env: EnvMap,
  cwd: Option<&PathBuf>,
) -> Result<BufferedTurn, FrontalCodeError> {
  ...
  while let Some(event) = rx.recv().await {
    match &event {
      FrontalCodeEvent::ItemCompleted { item } => items.push(item.clone()),
      FrontalCodeEvent::TurnCompleted {
        final_response: fr,
        usage: u,
        session_id: sid,
      } => {
  ...
  if !status.success() {
    ...
    return Err(FrontalCodeError::Exit {
      code: status.code(),
      stderr,
    });
  }
...
pub struct StreamedTurn {
  pub events: mpsc::Receiver<FrontalCodeEvent>,
...
impl StreamedTurn {
  ...
  pub async fn finish(mut self) -> Result<(), FrontalCodeError> {
...
pub async fn run_turn_streamed(
  ...
  cwd: Option<&PathBuf>,
  session_id: Arc<Mutex<Option<String>>>,
) -> Result<StreamedTurn, FrontalCodeError> {
  ...
  tokio::spawn(async move {
    while let Some(event) = rx.recv().await {
      if let FrontalCodeEvent::TurnCompleted {
        session_id: sid, ..
      } = &event
      {
