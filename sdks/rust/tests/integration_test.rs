...
use frontal_code_sdk::{
  FrontalCode, FrontalCodeError, FrontalCodeEvent, FrontalCodeOptions, InputEntry, ThreadInput,
  ThreadOptions, ThreadRunOptions,
};

use common::MockFrontalCode;
...
fn frontal_code_for(mock: &MockFrontalCode) -> FrontalCode {
  FrontalCode::new(FrontalCodeOptions {
...
async fn run_returns_buffered_turn() {
  let mock = MockFrontalCode::new(SINGLE_TURN, 0);
  let frontal_code = frontal_code_for(&mock);
  let turn = frontal_code
  ...
  assert_eq!(
    turn.items,
    vec![
      frontal_code_sdk::ThreadItem::Text {
        text: "thinking...".into()
      },
      frontal_code_sdk::ThreadItem::ToolUse {
        name: "edit".into(),
        input: "{}".into()
      },
    ]
...
async fn run_stores_session_id() {
  let mock = MockFrontalCode::new(SINGLE_TURN, 0);
  let frontal_code = frontal_code_for(&mock);
  let thread = frontal_code.start_thread(ThreadOptions::default());
  thread
    .run(
...
async fn global_config_and_base_url_flattened() {
  let mock = MockFrontalCode::new(SINGLE_TURN, 0);
  let frontal_code = FrontalCode::new(FrontalCodeOptions {
    command: Some(mock.bin_path.to_string_lossy().into_owned()),
    env: Some(mock.env()),
...
async fn non_zero_exit_is_an_error() {
  let mock = MockFrontalCode::new("", 2);
  let frontal_code = frontal_code_for(&mock);
  let result = frontal_code
    .start_thread(ThreadOptions::default())
    .run(
  ...
  assert!(matches!(
    result,
    Err(FrontalCodeError::Exit { code: Some(2), .. })
  ));
}
...
async fn unparseable_lines_ignored() {
  ...
  let mock = MockFrontalCode::new(&response, 0);
  let frontal_code = frontal_code_for(&mock);
  let turn = frontal_code
    .start_thread(ThreadOptions::default())
    .run(&ThreadInput::Text("x".into()), &ThreadRunOptions::default())
...
async fn run_streamed_yields_ordered_events() {
  ...
  while let Some(event) = streamed.events.recv().await {
    let name = match event {
      FrontalCodeEvent::TurnStarted => "turn.started",
      FrontalCodeEvent::ItemCompleted { .. } => "item.completed",
      FrontalCodeEvent::TurnCompleted { .. } => "turn.completed",
      FrontalCodeEvent::TurnFailed { .. } => "turn.failed",
    };
