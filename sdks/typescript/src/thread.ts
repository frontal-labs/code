...
export class Thread {
  private readonly frontal_code: FrontalCode;
  private readonly options: ThreadOptions;
  private sessionId?: string;

  constructor(frontal_code: FrontalCode, options: ThreadOptions, sessionId?: string) {
    this.frontal_code = frontal_code;
  ...
  async run(
    ...
    const turn = await collectTurn(
      streamEvents(this.frontal_code.command, args, env, this.options.workingDirectory),
    );
    if (turn.sessionId) {
  ...
  async runStreamed(
    input: ThreadInput,
    runOptions: ThreadRunOptions = {},
  ): Promise<{ events: AsyncGenerator<FrontalCodeEvent> }> {
    const args = this.buildArgs(input, runOptions);
    const env = this.frontal_code.buildEnv();
    const self = this;
    const generator = streamEvents(
      this.frontal_code.command,
      args,
      env,
      this.options.workingDirectory,
    );

    async function* wrap(): AsyncGenerator<FrontalCodeEvent> {
