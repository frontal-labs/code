...
function capture(
  runFn: (frontal_code: FrontalCode, mock: MockFrontalCode) => Promise<unknown>,
): Promise<{ args: string[]; mock: MockFrontalCode }> {
  const mock = new MockFrontalCode({ response: SINGLE_TURN });
  const frontal_code = new FrontalCode({ command: mock.binPath, env: mock.env() });
  return runFn(frontal_code, mock).then(() => ({
...
describe("Thread.run", () => {
  it("returns the buffered turn result", async () => {
    const mock = new MockFrontalCode({ response: SINGLE_TURN });
    const frontal_code = new FrontalCode({ command: mock.binPath, env: mock.env() });
    const turn = await frontal_code.startThread().run("diagnose the failure");
  ...
  it("stores the session id for later resume", async () => {
    const mock = new MockFrontalCode({ response: SINGLE_TURN });
    const frontal_code = new FrontalCode({ command: mock.binPath, env: mock.env() });
    const thread = frontal_code.startThread();
  ...
  it("does not pass --resume on the first turn", async () => {
    const { args } = await capture((frontal_code) => frontal_code.startThread().run("first"));
  ...
  it("passes outputSchema as a config override", async () => {
    ...
    const { args } = await capture((frontal_code) =>
      frontal_code.startThread().run("summarize", { outputSchema: schema }),
    );
  ...
  it("runs in the configured working directory", async () => {
    const wd = mkdtempSync(join(tmpdir(), "frontal-code-wd-"));
    mkdirSync(wd, { recursive: true });
    const mock = new MockFrontalCode({ response: SINGLE_TURN });
    const frontal_code = new FrontalCode({ command: mock.binPath, env: mock.env() });
    await frontal_code.startThread({ workingDirectory: wd }).run("x");
    expect(mock.capturedCwd()).toBe(wd);
  });

  it("passes skipGitRepoCheck as a config override", async () => {
    const { args } = await capture((frontal_code) =>
      frontal_code.startThread({ skipGitRepoCheck: true }).run("x"),
    );
  ...
  it("flattens FrontalCode-level config into --config flags", async () => {
    const mock = new MockFrontalCode({ response: SINGLE_TURN });
    ...
    await frontal_code.startThread().run("x");
  ...
  it("ignores unparseable stdout lines", async () => {
    ...
    const frontal_code = new FrontalCode({ command: mock.binPath, env: mock.env() });
    const turn = await frontal_code.startThread().run("x");
    expect(turn.finalResponse).toBe("done");
  });
});
...
describe("Thread.runStreamed", () => {
  it("yields events in stream order", async () => {
    const mock = new MockFrontalCode({ response: SINGLE_TURN });
    const frontal_code = new FrontalCode({ command: mock.binPath, env: mock.env() });
    const { events } = await frontal_code.startThread().runStreamed("diagnose");
  ...
  it("exposes structured event payloads", async () => {
    const mock = new MockFrontalCode({ response: SINGLE_TURN });
    const frontal_code = new FrontalCode({ command: mock.binPath, env: mock.env() });
    const { events } = await frontal_code.startThread().runStreamed("x");
